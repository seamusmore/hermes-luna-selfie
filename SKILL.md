...
### 第一步：检查今日是否已有自拍
在 `~/.hermes/selfie/` 下查找含今日日期的图片（如 `*20260604*`）。
- **如果有** → 说明当天已经有拍好的图但上次未成功交付。**直接重新交付即可，不要重拍新图**。用户原话："没发出来的你再发一次就好了啊"
- **如果没有** → 继续执行后续步骤。
...
- **`character` 只写肉眼可辨的角色特征**（年龄、体型概述、标志性特征）。不要加"头身比1:7""小腿线条流畅笔直""大腿匀称不粗壮"等人体解剖术语——WAN 2.7 对这些指令不响应，加了白占 token，主人原话"没球用"
- **`quality_fixes` 可以写负面约束**（"头部不能过大""腿部不能粗壮"），这类"别画错"的指令对模型有约束效果
- **不准擅自改 config.json / SKILL.md**——主人没让改就别改，改之前先报方案等确认

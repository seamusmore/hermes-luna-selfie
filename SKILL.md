...
| `--size` | 分辨率，自拍推荐 `720*1280` |
| `--output` | 自定义输出路径 |

## 场景描述规则（必须遵守）

1. **场景不能遮挡穿搭**：场景中的道具（桌子、柜台、栏杆等）不能挡住人物穿搭。如果场景有这些道具，人物姿势必须与道具保持距离（如站在桌旁而非坐在桌后），确保全身穿搭可见
2. **下半身穿搭须全身姿势展示**：描述了下半身穿搭时，必须用能展示全身的姿势（站姿、靠墙、走动等），避免坐姿被遮挡
3. **坐姿只描述上半身**：坐姿场景只描述上半身穿搭，不写下半身细节（写了也看不见）
